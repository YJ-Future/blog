...
- 老年代   
## 13 java内存模型  
及时编译器、处理器需要保证程序遵循as-if-serial属性。即在单线程情况下，要给程序一个顺序执行的假象，经过重排序的执行结果和顺序执行结果保持一致。  
如果两个操作之间存在数据依赖关系，那么及时编译器和处理器不能调整两者之间的顺序，避免改变程序的语义。  
### java内存模型和happens-before关系  
java内存模型中最重要的一个概念是happens-before关系。happens-before描述两个操作的内存可见性。如果X happens-before Y，则X的结果对于Y可见。  
在同一个线程中，字节码的先后顺序也是存在happens-before关系的，在程序控制流路径中，靠前的字节码happens-before靠后的字节码，但是不代表执行顺序  
上就一定是靠前的happens-before靠后的，如果靠后的字节码没有观察获取靠前的字节码的结果，及时编译器可能会重排序它们，实际上出现靠后的字节码先执行。  
happens-before的几种常见的情况：  
- 解锁操作happens-before之后对同一把锁的加锁操作。  
- volatile字段的写操作happens-before之后对该字段的读取。  
- 线程启动操作happens-before线程中第一个操作。  
- 线程对其他线程的中断操作happens-before被中断线程收到的中断事件（被中断线程的InterruptedException异常，或第三个线程对被中断线程的  
Thread.interrupted或者Thread.isInterrupted调用）。  
- 构造器中最后一个操作happens-before析构器的第一个操作。  
happens-before关系具有传递性。  
### java内存模型的底层实现  
java内存模型是通过内存屏障禁止重排序的。内存屏障的类型：读读类型、读写类型、写读类型、写写类型。  
及时编译器在编译生成机器码时，针对存在happens-before关系的情况，会插入相应的内存屏障，来限制及时编译器重排。  
比如volatile关键字修饰的字段，写操作之前的操作不能重排到这个写操作之后，这个字段的读操作之后的所有操作不能重排到这个读操作之前，volatile字段  
的写操作happens-before该字段的读操作。  
及时编译器会根据具体的底层体系架构，将内存屏障替换成具体的CPU指令。以x86_64架构为例，读读、读写、写写类型屏障是空操作，写读屏障会替换成具体的  
CPU指令。  
在X86_64架构上，只有volatile字段写操作之后的写读内存屏障需要替换成具体指令，指令的具体效果为强制刷新处理器的写缓存，写缓存是处理器用来加速内存  
存储效率的技术。  
在进行内存写操作时，cpu不会等待该指令结束，而是直接开始下一个指令，依赖写缓存将更改的数据同步到内存中。  
强制刷新写缓存，将当前线程写入volatile字段的值同步到内存中。内存写操作会无效化其他CPU中指向该写入的内存地址的缓存行，那么其他cpu就能立即看到  
这个volatile字段的最新值。  
解锁操作happens-before之后对同一把锁的加锁操作，解锁时也会强制刷新缓存，从而让当前线程修改的内存最新值对其他线程可见。    
volatile字段及时编译器无法把它分配到寄存器中，cpu缓存行中没有时，每次读取需要从内存中读取。  
## 14 jvm如何实现synchronized  
synchronized关键字用来对程序进行加锁，可以用来修饰代码块、静态方法、实例方法。  
synchronized代码块，编译成字节码会包含moniterenter和monitorexit指令。这两种指令都会使用一个引用类型数据，即锁对象。  
